...
fn test_simulator_backend() {
    ...
    // Allocate 3 qubits
    let alloc = backend.allocate(crate::backend::AllocationRequest {
        qubits: 3,
        logical_qubits: 0,
    }).unwrap();
    ...
    // Execute circuit
    let result = backend.execute(crate::backend::QuantumCircuitInfo {
        name: "test".to_string(),
        qubits: 3,
        gates: 5,
    }).unwrap();
    ...
    // Measure
    let measure_result = backend.measure(crate::backend::MeasurementRequest {
...
#[test]
fn test_swap_gate() {
    let gate = QuantumGate::Swap { qubit1: 0, qubit2: 1 };
    assert!(gate.is_two_qubit());
