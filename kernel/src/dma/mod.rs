...
impl DmaRing {
    ...
    pub fn len(&self) -> usize {
        self.tail.wrapping_sub(self.head) % self.capacity()
    }
    pub fn is_empty(&self) -> bool {
        self.head == self.tail
    }
    pub fn push(&mut self, d: u64) -> Result<(), DmaError> {
        let n = (self.tail + 1) % self.capacity();
        if n == self.head {
            return Err(DmaError::Full);
        }
        self.desc[self.tail] = d;
        self.tail = n;
        Ok(())
    }
    pub fn pop(&mut self) -> Result<u64, DmaError> {
        if self.is_empty() {
            return Err(DmaError::Empty);
        }
        let d = self.desc[self.head];
...
mod tests {
    ...
    #[test]
    fn dma() {
        assert!(DmaRegion::new(1, 4096).is_err());
        let mut r = DmaRegion::new(0x1000, 4096).unwrap();
        r.map();
        assert!(r.is_mapped());
    }
    #[test]
    fn ring_spsc() {
        let mut r = DmaRing::new(4).unwrap();
        r.push(1).unwrap();
