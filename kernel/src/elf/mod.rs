...
pub fn load(data: &[u8]) -> Result<ElfInfo, ElfError> {
    ...
    if data[4] != 2 { return Err(ElfError::BadClass); }
...
#[cfg(test)] mod tests { use super::*; #[test] fn reject() { assert_eq!(load(&[0; 10]), Err(ElfError::TooSmall)); } #[test] fn accept() { let mut b = vec![0u8; 64]; b[0..4].copy_from_slice(&MAGIC); b[4]=2; b[5]=1; b[18]=62; b[24]=0x10; assert!(load(&b).is_ok()); } }
