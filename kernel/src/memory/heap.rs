...
mod tests {
    ...
    #[test]
    fn bump() {
        let mut h = KernelHeap::new(0x1000, 0x1000);
        assert!(h.alloc(16, 8).is_some());
        assert!(h.alloc(0x2000, 8).is_none());
    }
    #[test]
    fn rejects_bad_align() {
        let mut h = KernelHeap::new(0x1000, 0x1000);
        assert!(h.alloc(8, 0).is_none());
        assert!(h.alloc(8, 3).is_none());
    }
    #[test]
    fn overflow_safe() {
        let mut h = KernelHeap::new(usize::MAX - 16, 32);
        assert!(h.alloc(64, 8).is_none());
    }
}
